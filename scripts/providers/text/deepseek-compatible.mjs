...
export function createDeepSeekTextProvider(env) {
  const model = env.AI_TEXT_MODEL || 'deepseek-chat';
  const baseUrl = env.AI_TEXT_API_BASE_URL || 'https://api.deepseek.com';
  const apiKey = env.AI_TEXT_API_KEY;
...
async function requestChatCompletion({ apiKey, baseUrl, model, messages }) {
  if (!apiKey || !baseUrl || !model) {
    throw new Error('Missing DeepSeek text translation configuration.');
  }
